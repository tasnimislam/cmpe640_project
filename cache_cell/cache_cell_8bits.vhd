...
end cache_cell_8bits;

architecture structural of cache_cell_8bits is
...
signal data_inside_bar: std_logic_vector(7 downto 0);
...
begin
  ...
  write_operation: Dlatch8 port map(d=> Write_Data, clk=> Write_enable, q=> data_inside, qbar=> data_inside_bar);

  invert_read_enable_signal: inverter port map(input=> Read_enable, output=> invert_read_enable);
  read_operation: tx8 port map(sel=> Read_enable, selnot=> invert_read_enable, input=> data_inside, output=> Read_Data);
